...
In [Chapter -@sec-projects], we'll discuss how to organize an R project, but
...
practice is to begin the file name with it's "type" (e.g. analysis, note,
...
are a seasoned veteran in the language. A good tip for using you `.Rprofile`
...
system, it's all for naught.

R is a relatively old language, so it has been updated
[many times](https://cran.r-project.org/bin/windows/base/old/) since launch.
...
to stop working on other peoples machines, a harder time googling bugs, and to
get stuck with old code.
